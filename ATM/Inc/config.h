...
#define TIMEOUT          40         // number of wrong / missing input commands before emergency off
                                     // (40 * DELAY_IN_MAIN_LOOP = 200ms; bumped from the original 25ms
                                     // default because a PC/USB-serial link has more jitter than a
                                     // dedicated Arduino loop -- ROS2 node sends at 50Hz/20ms)
...
// left sensor board cable (L_TX-PA2 / L_RX-PA3). ROS2 PC talks to the board here.
// Packet framing + checksum + feedback (measured speed, wheel odometry ticks, battery)
// mirror the protocol used by hoverboard-firmware-hack-FOC (see main.c SerialCommand/SerialFeedback).
#define CONTROL_SERIAL_USART2
#define CONTROL_BAUD       115200   // control via usart from eg an Arduino or raspberry
#define SERIAL_START_FRAME  0xABCD  // [-] Start frame definition for serial commands / feedback
#define ENABLE_ODOMETRY             // adds hall-sensor tick counters (wheelR_cnt/wheelL_cnt) to serial feedback
