...
// ###############################################################################
  ...
  #ifdef CONTROL_SERIAL_USART2
    uint32_t serial_feedback_counter = 0;
  #endif
  ...
  while(1) {
    ...
    {
      __disable_irq();
      Serialcommand cmd_snapshot = command;
      __enable_irq();

      uint16_t checksum = (uint16_t)(cmd_snapshot.start ^ cmd_snapshot.steer ^ cmd_snapshot.speed);
      if (cmd_snapshot.start == SERIAL_START_FRAME && cmd_snapshot.checksum == checksum) {
        cmd1 = CLAMP((int16_t)cmd_snapshot.steer, -1000, 1000);
        cmd2 = CLAMP((int16_t)cmd_snapshot.speed, -1000, 1000);
        timeout = 0;
      }
      // on a corrupted/desynced frame: do NOT reset timeout, so TIMEOUT safety stop still engages
    }
    #endif
    ...
    // ####### FEEDBACK SERIAL OUT (for ROS2 / odometry) #######
    #ifdef CONTROL_SERIAL_USART2
      if (serial_feedback_counter++ % 2 == 0) {  // every 2 loop iterations (~10ms @ DELAY_IN_MAIN_LOOP=5)
        ...
        #endif
        Feedback.batVoltage   = (int16_t)(batteryVoltage * 100.0f);
