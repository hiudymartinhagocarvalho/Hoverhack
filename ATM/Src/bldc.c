...
static int16_t hallTickDirection(int16_t prev, int16_t curr) {
  static const int16_t dir_lut[6] = {0, -1, -2, 0, 2, 1};
...
#ifdef ENABLE_ODOMETRY
    // Hall sensor tick counter (right, mirrored motor mount -> subtract direction)
    {
      uint8_t encoding  = (uint8_t)((hall_ur << 2) + (hall_vr << 1) + hall_wr);
      int wheel_pos     = rtConstP.vec_hallToPos_Value[encoding];
